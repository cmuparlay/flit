...
#ifndef ARAVIND_BST_DURABLE_AUTOMATIC_
#define ARAVIND_BST_DURABLE_AUTOMATIC_
...
class alignas(ALIGNMENT) AravindBstDurableAutomatic {
private:
    struct alignas(ALIGNMENT) Node {
        persist<int, flush_option::flush> key;
        persist<T, flush_option::flush> value;
        PERSIST<std::atomic<Node*>, flush_option::flush> right;
        PERSIST<std::atomic<Node*>, flush_option::flush> left;
...
public:
  AravindBstDurableAutomatic(int) : AravindBstDurableAutomatic() {}
  
  AravindBstDurableAutomatic() {
    Node* s;
    Node* inf0;
    Node* inf1;
    Node* inf2;
  ...
  bool add(int key, T val) {
      ...
      while (1) {
          ...
          PERSIST<std::atomic<Node*>, flush_option::flush>* child_addr;
          if (key < parent->key) {
  ...
  bool bst_cleanup(int key) {
      ...
      PERSIST<std::atomic<Node*>, flush_option::flush>* succ_addr;
      if (key < ancestor->key) {
      ...
      PERSIST<std::atomic<Node*>, flush_option::flush>* child_addr;
      PERSIST<std::atomic<Node*>, flush_option::flush>* sibling_addr;
      if (key < parent->key) {
         child_addr = &(parent->left);
  ...
  static std::string get_name() {
      return "Bst Auto, " + PERSIST<std::atomic<int>, flush_option::flush>::get_name();
  }
...
template <class T, template<typename, bool> typename PERSIST>
thread_local typename AravindBstDurableAutomatic<T, PERSIST>::seek_record_t AravindBstDurableAutomatic<T, PERSIST>::seek_record;

#endif /* ARAVIND_BST_DURABLE_AUTOMATIC_*/
