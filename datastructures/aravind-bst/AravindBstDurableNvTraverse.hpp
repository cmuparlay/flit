...
#ifndef ARAVIND_BST_DURABLE_NVTRAVERSE_
#define ARAVIND_BST_DURABLE_NVTRAVERSE_
...
class alignas(ALIGNMENT) AravindBstDurableNvTraverse {
private:
  struct alignas(ALIGNMENT) Node {
      ...
      void flush() {
        if(left.is_flush_needed() || right.is_flush_needed())
          FLUSH_STRUCT(this);
      }
  };

  struct seek_record_t{
      Node* ancestor_grandparent;
      Node* ancestor_parent;
      Node* ancestor;
      Node* successor;
      Node* grandparent;
      Node* parent;
...
public:
  AravindBstDurableNvTraverse(int) : AravindBstDurableNvTraverse() {}

  AravindBstDurableNvTraverse() {
    Node* s;
    Node* inf0;
    Node* inf1;
    Node* inf2;
  ...
  void bst_seek(int key){
    ...
    seek_record.ancestor_grandparent=seek_record_l.ancestor_grandparent;
    seek_record.ancestor_parent=seek_record_l.ancestor_parent;
  ...
  void bst_seek_leaf(int key){
      seek_record_t seek_record_l;
      Node* node_s = ADDRESS(root->left); // root is immutable
  ...
  bool contains(int key) {
     ...
     if(seek_record.grandparent) seek_record.grandparent->flush();
     seek_record.parent->flush();
     // make_persistent
     seek_record.leaf->flush();
  ...
  */
  void flush_path(int key, Node* node_start, Node* node_stop) {
      Node* current = node_start;
      Node* current_field = NULL;
  ...
  bool add(int key, T val) {
      ...
      while (1) {
          ...
          if(seek_record.ancestor_grandparent) seek_record.ancestor_grandparent->flush();
          if(seek_record.ancestor_parent) seek_record.ancestor_parent->flush();

          // make_persistent
          seek_record.ancestor->flush();
          flush_path(key, seek_record.successor, seek_record.parent);
          seek_record.parent->flush();
          seek_record.leaf->flush();
      ...
      MEM_BARRIER;
          ...
          Node* chld = child_addr->load(std::memory_order_seq_cst, flush_option::flush); 
  ...
  bool bst_cleanup(int key) {
      ...
      Node* chld = child_addr->load(std::memory_order_seq_cst, flush_option::flush);
      if (!GETFLAG(chld)) {
          chld = sibling_addr->load(std::memory_order_seq_cst, flush_option::flush);
          sibling_addr = child_addr;
      }
  //#if defined(__tile__) || defined(__sparc__)
      while (1) {
          Node* untagged = sibling_addr->load(std::memory_order_seq_cst, flush_option::flush);;
  ...
  //#endif
      ...
      Node* sibl = sibling_addr->load(std::memory_order_seq_cst, flush_option::flush);
  ...
  static std::string get_name() {
      return "Bst NvTraverse, " + PERSIST<std::atomic<int>, flush_option::flush>::get_name();
  }
...
template <class T, template<typename, bool> typename PERSIST>
thread_local typename AravindBstDurableNvTraverse<T, PERSIST>::seek_record_t AravindBstDurableNvTraverse<T, PERSIST>::seek_record;

#endif /* ARAVIND_BST_DURABLE_NVTRAVERSE_*/
