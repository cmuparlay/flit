...
#ifndef SKIPLIST_DURABLE_AUTOMATIC_HPP_
#define SKIPLIST_DURABLE_AUTOMATIC_HPP_
...
class alignas(ALIGNMENT) SkiplistDurableAutomatic {
private:
  ...
  struct alignas(ALIGNMENT) Node {
      ...
      persist<T, flush_option::flush> val;
      persist<unsigned char> toplevel;
      PERSIST<std::atomic<Node*>, flush_option::flush> next[MAX_LEVEL];
...
public:
  ...
  SkiplistDurableAutomatic(int) : SkiplistDurableAutomatic() {}

  SkiplistDurableAutomatic() {
    Node *min, *max;
  ...
  static std::string get_name() {
      return "Skiplist Auto, " + PERSIST<std::atomic<int>, flush_option::flush>::get_name();
  }
...
template<typename T, template<typename, bool> typename PERSIST> unsigned int SkiplistDurableAutomatic<T, PERSIST>::random_seed = 323423;
template<typename T, template<typename, bool> typename PERSIST> thread_local typename SkiplistDurableAutomatic<T, PERSIST>::Node* SkiplistDurableAutomatic<T, PERSIST>::nodes[1024];

#endif /* SKIPLIST_DURABLE_AUTOMATIC_HPP_ */