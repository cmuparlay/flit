...
private:
  ...
  struct alignas(ALIGNMENT) Node {
      ...
      persist<unsigned char, flush_option::no_flush> toplevel;
...
public:
  ...
  SkiplistDurableManual(T val) {
    ...
    min = static_cast<Node*>(ssmem.alloc(sizeof(Node)));
    //min = static_cast<Node*>(malloc(sizeof(Node)));
    new (min) Node(INT_MIN, val, max, MAX_LEVEL);
    FLUSH_STRUCT(max);
    FLUSH_STRUCT(min);
    this->head = min;
    FENCE();
...
#endif /* SKIPLIST_DURABLE_MANUAL_HPP_ */
